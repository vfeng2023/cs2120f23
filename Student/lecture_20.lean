/-!
  states that there exists some w of type α which makes a preddicate a true proposition
  To prove, provide and example.
  Ex: ∃ n: ℕ, n%2=0
  Proof:
  example: ∃ (n: Nat), n%2=0:= Exists.intro 2 rfl
...
#check (∀ (c: Cat), (∀ (d: Cat), Likes₃ c d)) → (∀ (c d: Cat), Likes₃ c d)

/-!
Finally, give a formal proof in Lean of the proposition in problem #5.
-/

example : (∀ (c: Cat), (∀ (d: Cat), Likes₃ c d)) → (∀ (c d: Cat), Likes₃ c d) :=
λ cats_like_other_cats => (λ (c d: Cat) => cats_like_other_cats c d)
