/-!
## TLDR:
- basically Prop has 1to1 correspondence with Type universe
- excluded middle is an axiom that states either P or not P is true
...
| Or.inl a =>
  match bornotb with
    | Or.inl b => Or.inl (λ _:A => nab (And.intro a b))
    | Or.inr nb => Or.inr nb
| Or.inr na => Or.inl na
