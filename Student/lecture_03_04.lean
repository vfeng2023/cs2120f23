...
#eval id_nat 7 -- name of func space argument
#eval id_string "Hello"
#eval id_bool true
#eval id_string "Most seductive"
...
-/
--polymorphic - Take arguments of many different types
def id_poly : (α : Type) → α → α --α limited scope for first line --takes a type and an argument of that type and returns a value of that type
| α, v => v 
--| _, v => v--or

/-def id_poly' (α : Type) : α → α --α first arg is consumed
| v=>v -/
/-
OR
def id_poly: (α: Typ)
-/
-- function takes a type as a function and then takes a value of that type
/-
...
-- each type arg specializes to a function of that type
--#eval id_poly Nat false 
--#eval id_poly Bool "Hello"
#eval id_poly Bool true
/-!
...
#eval @id_poly' Nat 7          -- α = Nat, explicitly not allowed
#eval id_poly' "Hello!"   -- α = String, inferred!
#eval id_poly' true       -- α = Bool, inferred!
...
#eval square 69
#eval apply2_nat square 69

#eval square 4
#eval apply2_nat square 16
...
#eval is_even_len "Love" --True
#eval is_even_len "Love!" --False
#eval is_even_len "Love!!" --True
...
--No -- the explicit invocation has just been replaced with an implicit one
/-!
...
def is_even_len'' := (is_even ∘ String.length) --\comp get you ∘ 
