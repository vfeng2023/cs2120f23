...
-/
--Doll = the set of objects that can be build with a finite application of the constructor
inductive Doll : Type
| solid
| shell (d : Doll)
...
*solid*, otherwise, if *d* is a nested doll, then it's of the form (shell d')
for some doll d', it must return the d'. Then verify using #reduce
...
| Nat.succ n, y => Nat.succ (nat_add n y)
