...
Nat → Nat: type
Nat -> Nat → Nat: type
All of them are of type type
String → String -> string is right associative, so the parens go on the last two strings
On the other hand, a function like String.append "owo" "uwu" is left associative (think of pacman)
-/
...
#check (f1)     
/-String → String (adding parenthesis gives pretty output)-/
/-
...
- Are #1 and #3 equivalent? NOOO
- Give English explanations of these function types
#1 type of a function that takes a function which takes a string and outputs a string as argument, then outputs a string
#2 type of a function that takes a string as input and then outputs a function that takes a string as input and a string as output
#3 takes a two strings as input and then outputs a string
- Give some examples of functions of these types

# 1 python fname.__name___ > gets name of function
# 2 concatenation
-/
...
# Produces nor (by de morgan's laws or looking at true tables)
-/
...
#eval mystery true true
#eval mystery true false
#eval mystery false true
