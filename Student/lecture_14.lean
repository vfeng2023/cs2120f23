...
-- unsatisfiable = true for no interpretations = is_valid (¬ E)

--recursive method for generating list of bools
--have a general idea of how the code is set up
end TLDR
/-!
...
--result = make bool list n'
--for every n'th object: add to
def make_bool_lists': Nat → List (List Bool)
| 0 => [[]]
