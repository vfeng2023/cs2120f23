...
def pythag (a: Nat) (b: Nat) (c: Nat) : Bool:= (a^2 + b^2==c^2)
...
def or_prods_to_prod_ors (α β γ δ: Type): α × γ ⊕ α × δ ⊕ β × γ ⊕ β × δ → (α ⊕ β) × (γ ⊕ δ)
| Sum.inl (a,g) => (Sum.inl a, Sum.inl g)
| Sum.inr ( Sum.inl (a,d)) => (Sum.inl a, Sum.inr d)
| Sum.inr (Sum.inr (Sum.inl (a,g))) => (Sum.inr a, Sum.inl g) 
...
-/
--expression: (A ∨ O) ∧ (B ∨ C) => (A ∧ B) ∨ (A ∧ C) ∨ (O ∧ B) ∨ (O ∧ C)
/-!
...
/-!
  Result of checking with is_valid (see other file for results)
